...
| `/` | **不做**，至少不进 P2 | firmlink 双计、`/System`、TCC、网络卷、可能几十分钟 I/O。要做必须单独确认，且不能和 `~` 并列成第三个 chip |
...
重尾正是读者要看的，轻尾照旧砍掉。可再生表固定 8 行（「清除列出的 N 项」的 N 要小而稳定）。

- **可再生**（`kind = Tag`）：这一组带删除控件（见「删除」）；标签措辞仍用「缓存」或
  「可再生」而非「可清除」——TAG 只保证可再生，不保证此刻删了也没事
  （`target` 里可能有 rustc 在写），这层风险写在确认文案里。
- **猜测**（`kind = Heuristic`）：另一套更淡的标签，和 TAG 必须能分开。
- **大目录**、**走树才见到的文件**。
- 行尾：「在 Finder 中显示」（`bigfiles::reveal`）。
- 口径进 tooltip：物理块；克隆 / 硬链接会重复计；加总不是卷用量。

**下钻（已落地）**：点某一目录行 → 以该 path 为 root 再扫。不持有全树，所以「展开」
唯一诚实的来源就是对那棵子树再走一遍（通常秒级）。外层结果压栈保留在内存
（每层只是几 KB 的 `ScanResult`），「← 返回上一级」即时恢复、不重扫；
下钻中的「取消」也回落到外层而非 Off。行内的 Reveal / 删除按钮 `stop_propagation`，
不触发下钻；扫描中的 Partial 表行是惰性的；文件行永不下钻。
chip 的「重新分析」始终指整个 `~`（清栈）——离开下钻用「返回」，不用重扫。
子扫描不写缓存（与 v1 一致）。
...
- 批量按钮**不叫「清除所有」**：排名表封顶 `TABLE_CAP` 行，它只能清列出的这几项，
  不是全盘所有带 TAG 的目录——文案如实写「列出的 N 项」。
