...
`mdfind -onlyin $HOME`——**与分析同域**）。两个 chip 都嵌在「/」卷的标题行旁，
视觉上暗示整卷，所以大文件的计数行以「~ ·」开头、空态写明「个人目录的索引内」
——范围不能只活在 tooltip 里。
...
| 用户自选目录 | **已落地** | 卷卡片「分析范围」行的文件夹按钮（`NSOpenPanel`，只允许目录），选定即开始分析。范围行在走树进行中整体隐藏——中途换范围必须先取消，绝不静默重启。选定的根对本次会话生效——「重新分析」此后指它，「清除结果」恢复默认 `~`。面板会因选择器抢焦点而自动隐藏——走树本就 hide 不断，重开即见 |
...
`trash_regenerable` 摘行时当前视图、返回栈、磁盘缓存三处同步剪掉；(2)「整棵重扫太贵」
...
任何字；下钻视图与改根后的首扫 root 对不上基线，整体不显示 Δ——半张可比的表
读起来像「其余都没变」。「清除结果」连 `.prev` 一起删。
