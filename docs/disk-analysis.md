...
| `~/Library` | **已落地**（预设 chip） | 盲区特写，便宜一半以上。选择器旁的一键 chip，走 `start_disk_analysis_at` 的既有通道 |
| 缓存根集合 | **已落地**（预设 chip） | `~/Library/Caches`、`~/.cache`、`~/Library/Developer` 三条显式路径的**合并走树**（`ScanScope`，多根）：一份结果、一个进度流，dirs 表把各根的 level-1 并成一张排名，base（`~`）只作标签基准。缓存身份与 Δ 基线按 roots 集合区分——绝不与 `~` 全量走树共用一份文件；不存在的路径（多数机器无 `~/.cache`）跳过不报错，但身份按**请求集合**原样记，中途 mkdir 不会孤儿化缓存。caption 逐条列出根（`a + b + c`），不冒充全家分析 |
...
`±X`（`DIFF_FLOOR` 10 MiB 起显，展示性阈值）；caption 注明「± 较上次分析（X 前）」。
