...
| 缓存根集合 | **已落地**（预设 chip） | `~/Library/Caches`、`~/.cache`、`~/Library/Developer`，加上家目录直下、全量走树里会被项目噪声淹没的工具缓存 `~/.npm`、`~/.cargo/registry`、`~/.gradle`、`~/.m2`——七条显式路径的**合并走树**。准入标准：知名位置且整体是缓存（取 `~/.cargo/registry` 而非 `~/.cargo`——`bin` 是安装的软件）。**这份清单就是 preset 的缓存身份**：每次修改都会换缓存文件名，旧结果与 Δ 基线孤儿化一次，扩集要一次到位（`ScanScope`，多根）：一份结果、一个进度流，dirs 表把各根的 level-1 并成一张排名，base（`~`）只作标签基准。缓存身份与 Δ 基线按 roots 集合区分——绝不与 `~` 全量走树共用一份文件；不存在的路径（多数机器无 `~/.cache`）跳过不报错，但身份按**请求集合**原样记，中途 mkdir 不会孤儿化缓存。caption 逐条列出根（`a + b + c`），不冒充全家分析 |
