...
盲区是接受的边界而不是缺陷：点开头的隐藏目录和 `~/Library` 大部分不进索引，海量小文件的目录也没有单个大文件可找——那些属于第二步的目录分析器（后台走树，已落地，见 docs/disk-analysis.md）。删除按钮走 `confirm.rs` 确认后调 `NSFileManager.trashItemAtURL`——Finder 自己的「移到废纸篓」，清空前可恢复，绝不直接 unlink；这与 `terminate.rs` 同一姿态：面板只发出可拒绝、可撤销的请求。查询结果属于「此刻在问什么」的瞬态状态，**新建磁盘空间窗口时**重置——原先挂在面板隐藏上，而打开这个窗口恰好就会让面板失焦收起，那样一开窗就把自己清空了。
