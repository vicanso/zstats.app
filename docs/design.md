...
界面页的「阻止 Mac 休眠」开关持有一条 IOKit 电源断言，开着就一直握着，关掉立刻释放。这是本应用**第三处**对系统动手的地方，也是刻意最轻的一处：它不需要 `confirm.rs` 的确认框，因为它不改变机器上的任何东西，再点一次就撤销，而且活不过本进程——持有者退出（包括崩溃）时内核自己会收回断言。前两处（移入废纸篓、请求退出）改的是别人的数据和别人的进程，这一处只改本机的空闲休眠策略。

- **取 `PreventUserIdleSystemSleep`，不取 `PreventSystemSleep`**。后者只有接着电源时才被尊重，而一个到电池上就悄悄失效的开关，比没有这个开关更糟。前者就是 `caffeinate -i` 拿的那条。
- **只管系统休眠，不管屏幕**。挡屏幕息屏是另一条断言、另一个决定；一个开关只承诺一件事。
- **合盖照样睡**。没有哪条断言管得住合盖，所以这一条必须写在开关的说明里——否则它一定会被当成 bug 报回来。
- **持久化，但必须在屏幕上看得见**。开着它通常是为了让某个长任务跑完，重启后静默失效是最坏的结果，所以 `app.toml` 记住它、启动时重新施加。代价是它不能是一个看不见的状态：开着时面板底栏挂一个杯子，点一下就关——退出的路不该要求先找到打开它的那一页。
- **两端都记 INFO 日志，断言名带上应用名**。和托盘恢复时钟同一个理由：「为什么我的 Mac 不睡了」是对着装好的构建问的，那里抓不到 DEBUG；而 `pmset -g assertions` 会直接点名持有者是 zstats。

状态就是那条断言的 id，空值用 IOKit 自己的 `kIOPMNullAssertionID`，所以「没有断言」不需要第二种表示；重复施加同一个状态什么也不做，不会漏出第二条断言。
