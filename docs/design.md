...
两种「缺数据」严格分开（和 History 不画折线是同一条诚实原则）：**采集器没报告的分钟**（睡眠、启动前）不可知，比较的两侧都不算；**报告了但这棵树不在 groups 里**说明它在采集器 top-N 之下——真的安静，按零计。后者正是「从无到有」能记满全程涨幅的原因。
...
**同一套环再喂一次内存（`state.mem_trend`）**，问的是泄漏：一个进程 footprint 一小时从 300 MB 涨到 1.5 GB、还没碰到 `alert-mem`，规则引擎按定义看不见，而这正是「看见时已经晚了」的那类事。单位是 MB（`trend::MIB`），u16 槽封顶 ~64 GB——就是整机。读取端不是 `rise` 而是 `climb`：**最新五分钟对本小时最早上报的五分钟**，量的是整段爬升，按小时均值做基线会把一条匀速爬升腰斩；另外两条门槛都在 `trend.rs`：至少上报 20 分钟才给结论（footprint 本该慢慢动，五分钟对它不是趋势），以及最新几分钟必须仍在小时最高值的 90% 以上（`CLIMB_HOLD`）——涨过又释放的是结束了的尖峰，不是泄漏。呈现是内存卡底部一条弱化的行（`mem_climb_strip`，`Chrome +1.2 GB · Code +420 MB`），门槛 `MEM_RISE_FLOOR` = 256 MB 在 `views/` 里，没人在涨就不占高度；一小时涨过 `CREEP_NOTIFY_BYTES`（1 GB）走 `notify::post_memory_creep`，和持续负载同款静默横幅，`creep_notified` 做再武装集合——爬升回落到门槛以下才会再报，所以一个一直漏的进程一小时说一次而不是每 tick 一次。仍然只展示、不评估：比较的两端都是 zstats 的 `phys_footprint_bytes`。横幅在外期间，Alerts 页有一张与持续负载卡同款的只读卡承接（`state::creeps_active`——`creep_notified` 升级成 name → 首次点名时刻的 map，卡按这个时刻插进时效序）：横幅点击一律落在那一页，落在一张不提爬升者的页面上，通知读作指向了空处；爬升回落、横幅再武装，卡也随之撤下。armed 列表同时点名这个观察器（一小时 ≥1 GB），否则那张卡和横幅像凭空出现。
