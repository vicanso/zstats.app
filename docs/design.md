...
存储是每棵应用树 60 个分钟槽、u16 单核百分比（`u16::MAX` 作无数据哨兵），每分钟取**最大值**——「在涨」问的是够到过哪，取均值会让分钟内一个空闲样本把真实的爬升拉回去。约 100 棵树共 ~20 KB。按树名做键，接受同名合流：root pid 会被每次重启打断，pid 复用还会把两个程序接成一条曲线（和告警卡片用 `SeenAlert::live` 把门是同一个理由）。
