//! Outbound-proxy resolution for the clean-hints update fetch — ported
//! from zedis (same author, same shape), minus its tracing.
//!
...
/// (`prefs::proxy`) at startup and on every save. A process global
/// because the HTTP caller runs on the background executor with no `cx`
/// to read the store through. Values:
...
pub fn set_configured_proxy(value: &str) {
    *CONFIGURED_PROXY
        .write()
        .expect("proxy setting lock poisoned") = value.trim().to_string();
}
    ...
    if !configured.is_empty() {
        match Proxy::new(configured) {
            ...
            Err(_) => {
                ...
                // app.toml can still hold junk — degrade to the system
...
pub fn app_proxy() -> Option<Proxy> {
    let configured = CONFIGURED_PROXY
        .read()
        .expect("proxy setting lock poisoned")
        .clone();
    ...
    for line in output.lines() {
        ...
        match parts.next() {
            Some("ProxyEnable") => {
                enabled = parts
                    .next_back()
                    .is_some_and(|v| v.trim_start_matches("0x") == "1");
            }
            Some("ProxyServer") => {
...
mod tests {
    ...
    fn scutil_prefers_https_then_http() {
        let both = "<dictionary> {\n  HTTPEnable : 1\n  HTTPPort : 7890\n  HTTPProxy : 127.0.0.1\n  HTTPSEnable : 1\n  HTTPSPort : 7891\n  HTTPSProxy : 10.0.0.2\n}\n";
        assert_eq!(
            proxy_from_scutil(both).as_deref(),
            Some("http://10.0.0.2:7891")
        );

        let http_only = "<dictionary> {\n  HTTPEnable : 1\n  HTTPPort : 7890\n  HTTPProxy : 127.0.0.1\n  HTTPSEnable : 0\n}\n";
        assert_eq!(
            proxy_from_scutil(http_only).as_deref(),
    ...
    fn wininet_bare_and_per_scheme_servers() {
        ...
        assert_eq!(
            proxy_from_wininet_reg(bare).as_deref(),
            Some("http://127.0.0.1:7890")
        );
